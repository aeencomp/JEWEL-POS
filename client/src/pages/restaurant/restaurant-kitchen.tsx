...
type OrderItem = { id: number; name: string; quantity: number; notes: string | null };
type Order = {
  id: number; orderNumber: string; status: string; source: string;
  customerName: string | null; table: { tableNumber: number } | null;
  items: OrderItem[];
};
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          return (
            <Card key={order.id} className="border-2 border-orange-200 dark:border-orange-900">
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {order.table ? `${isAr ? "طاولة" : "Table"} #${order.table.tableNumber}` : order.customerName || (isAr ? "طلب QR" : "QR Order")}
                      {order.source === "qr" && <Badge className="ms-2 text-[10px]">QR</Badge>}
                    </p>
                  </div>
                  <Badge className={`${sl.color} text-white`}>{isAr ? sl.ar : sl.en}</Badge>
                </div>
                ...
                <Button className="w-full bg-orange-600 hover:bg-orange-700" size="sm" onClick={() => updateStatus.mutate({ id: order.id, status: nextStatus(order.status) })}>
        ...
        {active.length === 0 && <p className="text-muted-foreground col-span-full text-center py-12">{isAr ? "لا طلبات نشطة" : "No active orders"}</p>}
      </div>
    </div>
  );
}
