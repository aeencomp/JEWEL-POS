...
import {
  ...
  Pill, ChevronRight, ArrowLeft, FilePlus2,
} from "lucide-react";

type BrandingData = { brandColor: string | null; logoUrl: string | null; name: string };

const navItems = [
  { path: "/pharmacy", icon: LayoutDashboard, label: "Dashboard", labelAr: "لوحة التحكم", exact: true },
  { path: "/pharmacy/pos", icon: ShoppingCart, label: "POS", labelAr: "نقطة البيع" },
  { path: "/pharmacy/inventory", icon: Package, label: "Drugs", labelAr: "الأدوية" },
  { path: "/pharmacy/prescriptions", icon: FilePlus2, label: "Prescriptions", labelAr: "الوصفات" },
  { path: "/pharmacy/expiry", icon: AlertTriangle, label: "Expiry Alerts", labelAr: "تنبيهات الصلاحية" },
  { path: "/pharmacy/customers", icon: Users, label: "Customers", labelAr: "العملاء" },
  { path: "/pharmacy/orders", icon: ClipboardList, label: "Sales", labelAr: "المبيعات" },
  { path: "/pharmacy/reports", icon: FileText, label: "Reports", labelAr: "التقارير" },
  { path: "/pharmacy/stock-audit", icon: ClipboardCheck, label: "Stock Audit", labelAr: "جرد المخزون" },
  { path: "/pharmacy/branding", icon: Palette, label: "Branding", labelAr: "العلامة التجارية" },
  { path: "/pharmacy/backup", icon: HardDrive, label: "Backup", labelAr: "النسخ الاحتياطي" },
];
    ...
    >
      <div className="px-5 pt-6 pb-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="font-bold text-sm text-white truncate">{branding?.name || posSystemLabel("pharmacy", isAr)}</p>
            <p className="text-[11px] text-teal-200/60">{posSystemSubtitle("pharmacy", isAr)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-white/10 space-y-2">
        <StoreSubscriptionPanel
          posSystem="pharmacy"
          variant="dark"
          buttonClassName="bg-teal-600 hover:bg-teal-500 text-white"
        />
        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-teal-200/50">{isAr ? "صيدلية" : "Pharmacy"}</p>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        {isImpersonating && (
          <div className="flex items-center justify-between gap-2 px-4 py-2 bg-teal-600 text-white text-sm">
