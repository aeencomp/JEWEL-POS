...
export default function AdminRestaurants() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      form.reset();
      toast({ title: t("restaurants.createdSuccess") });
    },
    onError: (error: Error) => {
      toast({ title: t("restaurants.failedCreate"), description: error.message, variant: "destructive" });
    },
  });
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/restaurants"] });
      toast({ title: t("restaurants.statusUpdated") });
    },
  });
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-page-title">{t("restaurants.title")}</h1>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>{t("restaurants.addNew")}</DialogTitle>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit((v) => createMutation.mutate(v))} className="space-y-4 mt-2">
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="name" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder={t("restaurants.restaurantName")} data-testid="input-restaurant-name" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="ownerName" render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("restaurants.ownerName")}</FormLabel>
                      <FormControl><Input placeholder={t("restaurants.ownerName")} data-testid="input-owner-name" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="phone" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="+964 xxx xxx xxxx" data-testid="input-phone" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <FormField control={form.control} name="email" render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("restaurants.email")}</FormLabel>
                ...
                <FormField control={form.control} name="address" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder={t("restaurants.address")} data-testid="input-address" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="plan" render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("restaurants.subscriptionPlan")}</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-plan">
                          <SelectValue placeholder={t("restaurants.selectPlan")} />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectItem value="basic">{t("subscriptions.basicPlan")} - 35,000 IQD</SelectItem>
                        <SelectItem value="standard">{t("subscriptions.standardPlan")} - 75,000 IQD</SelectItem>
                        <SelectItem value="premium">{t("subscriptions.premiumPlan")} - 125,000 IQD</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
                ...
                <div className="border-t pt-4 mt-4">
                  <p className="text-sm font-medium mb-3">{t("restaurants.loginCredentials")}</p>
                  <div className="grid grid-cols-2 gap-4">
                    <FormField control={form.control} name="username" render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("auth.username")}</FormLabel>
                        <FormControl><Input placeholder={t("restaurants.loginUsername")} data-testid="input-username" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="password" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input type="password" placeholder={t("restaurants.loginPassword")} data-testid="input-password" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-lg font-medium text-muted-foreground">{t("restaurants.noRestaurantsFound")}</p>
            <p className="text-sm text-muted-foreground mt-1">
              {search ? t("restaurants.tryDifferentSearch") : t("restaurants.addFirstToStart")}
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <CardContent className="p-5">
                  ...
                  {sub && (
                    <div className="flex items-center justify-between gap-2 p-2.5 rounded-md bg-muted/50 mb-3">
                      <div>
                        <p className="text-xs font-medium capitalize">{sub.plan} {t("dashboard.plan")}</p>
                        <p className="text-xs text-muted-foreground">{parseInt(sub.pricePerMonth).toLocaleString()} IQD</p>
                      </div>
                  ...
                  >
                    {restaurant.isActive ? (
                      <><PowerOff className="h-3 w-3 me-2" />{t("restaurants.deactivate")}</>
                    ) : (
                      <><Power className="h-3 w-3 me-2" />{t("restaurants.activate")}</>
                    )}
                  </Button>
                </CardContent>
              </Card>
